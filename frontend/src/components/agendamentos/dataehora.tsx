...
export default function DataHora({ setDataSelecionada, setHorarioSelecionado }: DataHoraProps) {
  ...
  // Buscar agendamentos (AGORA GUARDA DATA COMPLETA)
 ...
 function aoSelecionarDia(data: Date | undefined) {
  ...
  const horariosOcupadosNoDia = agendamentosOcupados
    .filter((agendamento) => agendamento.startsWith(dataFormatada))
    .map((agendamento) => agendamento.split(" ")[1])

  const livres = horariosPossiveis.filter(
    (horario) => !horariosOcupadosNoDia.includes(horario)
  )
      ...
      {dataSelecionadaCalendario && (
        <div className="mt-6 w-full max-w-md">
          <h2 className="text-center font-bold text-md text-black">
            ...
            {dataSelecionadaCalendario.toLocaleDateString("pt-BR")}
          </h2>
